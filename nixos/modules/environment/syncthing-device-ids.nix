...
in {
  options.environment.syncthingDeviceIds = mkOption {
    ...
    description = ''
      ...
      fingerprint of that host's syncthing-<host>-cert.age certificate, and
      so is public data.

      This module is only the accessor: the IDs live in
      secrets/syncthing-device-ids.nix. Configurations reference an entry as
